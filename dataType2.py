...
# 딕셔너리
# 딕셔너리는  데이터를 키와 value 로 저장하는 자료형 이다. 
...
print( m[0]  ) 
print( m[1]  )
#  m[0] ->  이름인지,  나이인지,  직업인지  알수 가 없다.  

info = { "name" : "이순신" , "age":20 , "job":"군인" , "name":"김유신" }
print( info["name"] )
...
if "김유신" in info.values():# 딕셔너리에서 valye 가져오기
    print("김유신있다.")
...
set2.update( ["정약용","문익점","이성계"] )  # 여러개 추가
print( set2 )

set2.remove("한석봉")  # 삭제  없는거 삭제하면 오류
print( set2 )
...
# 교집합, 합집합, 차집합
a = {"복숭아","배", "메론","체리", "포도"}  #도헌이가 좋아 하는 과일
...
'''
    ...
    tuple - 다수의 데이터 저장 가능, 중복허용, 추가수정삭제 불가능
...
while True:
    ...
    if menu == "1":
        ...
        name = input("판매할 과일 이름 : ").strip()
        count = int( input("판매 수량 : ").strip() )
        found = False # 판매할 과일 이 과일 데이터에 있냐 없냐
        for fruit in fruits :
            if fruit["name"] == name:
                found = True
        ...
        if not found : # 과일데이터에 판매할 과일이 없는경우 - not 연산자에 의해 거짓 <-> 참
            print("해당과일이 없습니다.")


    elif menu == "4":
        # 과일들의 재고량을 확인하여 5개 이하인 과일들을 찾아서 출력하기
        # 만약 5개 이하 인 과일이 없다면 재고 부족 과일이 없습니다. 라고 출력
        
        #컴프리헨션 으로 [ 표현식 for 변수 in 반복대상]
        ...
        # 오늘의 추천 과일 - 랜덤
        # inport random - randint(1, 10): 1~10사이 정수, randrange(1, 10) :1~9사이 정수
        # choice(리스트) : 리스트 내분의 값중 하나 선택
        # choices(리스트, k=3) : 리스트 안에서 3개를 선택(중복허용)
        # sample(리스트, 4) : 리스트안에 값중 4개를 선택 (중복 없음)
        # shuffle(리스트) : 리스트안에 값의 순서를 랜덤하게 섞음
        # choices(리스트, 가중치 , k=5) : 리스트 안에 값의 확률(가중치)을 적용하여 5개 선택
        # i=[1,2,3,4,5]
        # w=[0.1,0.2,0.3,0.1,0.2]
        # choices( i, 2, k=3 )
